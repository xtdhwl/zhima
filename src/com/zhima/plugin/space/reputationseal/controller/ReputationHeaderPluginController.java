...
public class ReputationHeaderPluginController extends BaseViewPluginController {
	...
	private ReputationHeaderViewPlugin viewPlugin;
	...
	public ReputationHeaderPluginController(BaseActivity activity, ReputationHeaderViewPlugin viewPlugin) {
	...
	public void loadData(ZMObject zmObject, boolean refreshed) {
		mZMOrganizationObject = (ZMOrganizationObject) zmObject;

		mAlbumRefreshList = mZMSpaceService.getCacheFocusImages(mZMOrganizationObject);
		if (mAlbumRefreshList.isEmpty()) {
			mZMSpaceService.getFocusImages(mZMOrganizationObject, false, this);
		} else {
	...
	private void setListener() {
		setSeekBarListenr();
		viewPlugin.setPicClickListener(mMultimediaClickListener);
	...
	private void setView() {
		viewPlugin.setNameView(mZMOrganizationObject.getName());
		viewPlugin.setZMIDView("ID:" + mZMOrganizationObject.getZMID());
		viewPlugin.setSignatureView(mZMOrganizationObject.getDescription());
		viewPlugin.setDefalutBackgroundResource(mDefalutAlbumResId);

		User user = mZMOrganizationObject.getUser();
		if (user != null) {
		...
		mHttpImageLoader.loadImage(mZMOrganizationObject.getImageUrl(), viewPlugin.getPhotoView(),
	...
	private void setAlbumView(ArrayList<ZMObjectImage> dataList) {
		...
		if (mAlbumAdapter == null) {
			mAlbumAdapter = new AlbumAdapter(mParentActivity, mDataList, mDefalutAlbumResId, 1, maxListSize);
			viewPlugin.setAdapter(mAlbumAdapter);
		} else {
	...
	private OnClickListener mMultimediaClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			intent.putExtra(BaseActivity.ACTIVITY_EXTRA, mZMOrganizationObject.getId());
	...
	private OnClickListener photoClickListener = new OnClickListener() {
		@Override
		public void onClick(View v) {
			Intent it = new Intent(mParentActivity, ReputationSpaceIntroActivity.class);
			it.putExtra(BaseActivity.ACTIVITY_EXTRA, mZMOrganizationObject.getId());
			startActivity(it);
		}
	};
	...
	private void addPraiseClick() {
		if (mZMOrganizationObject != null) {
			startWaitingDialog();
			mZMSpaceService.doPraise(mZMOrganizationObject, this);
		} else {
			ErrorManager.showErrorMessage(mParentActivity);
		}
	}
						...
						if (xDistance > yDistance) {
							...
							if (isUp) {
								...
								if (!isStartActivity) {
									...
									diaryIntent.putExtra(mParentActivity.ACTIVITY_EXTRA, mZMOrganizationObject.getId());
								...
								if (!isStartActivity) {
									...
									vedioImageIntent.putExtra(mParentActivity.ACTIVITY_EXTRA,
											mZMOrganizationObject.getId());
			...
			public void onStopTrackingTouch(SeekBar seekBar) {
				if (mProgress < 15) {
					if (!isStartActivity) {
						...
						retrievalIntent.putExtra(mParentActivity.ACTIVITY_EXTRA, mZMOrganizationObject.getId());
