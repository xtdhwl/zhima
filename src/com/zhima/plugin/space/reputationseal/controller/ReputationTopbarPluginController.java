...
/**
 * @ClassName: ReputationTopbarPluginController
 * @Description: 商业空间标题栏
 * @author luqilong
 * @date 2013-1-5 上午11:36:33
 * 
 */
public class ReputationTopbarPluginController extends BaseViewPluginController {

	private TopbarBaseViewPlugin mTopbarViewPlugin;
	private ZMOrganizationObject mZMOrganizationObject;

	public ReputationTopbarPluginController(BaseActivity activity, BaseViewPlugin parent) {
	...
	public void loadData(ZMObject zmObject, boolean refreshed) {
		...
		//判断是否为自己 
		User zmUser = mZMOrganizationObject.getUser();
	...
	public void favoriteTopbaClick(View v) {
		...
		if (mZMOrganizationObject != null) {
			...
			ContactService.getInstance(mParentActivity).addContact(entry, false, ReputationTopbarPluginController.this);
		} else {
	...
	private View.OnClickListener noticeTopbarClick = new View.OnClickListener() {
		...
		public void onClick(final View v) {
			if (mZMOrganizationObject != null) {
				...
				it.putExtra(BaseActivity.ACTIVITY_EXTRA, mZMOrganizationObject.getZMCode());
				startActivity(it);
			} else {
	...
	private void setTopbar() {
		...
		((TextView) topbar.findViewById(R.id.txt_topbar_title)).setText(mTopbarViewPlugin.getTitle());
	}
}
