 ...
 */
public class BusinessRecommendedPluginController extends BaseViewPluginController {
	/** 其他知天使个数 */
	...
	private BusinessRecommendedViewPlugin mViewPlugin;
	...
	public BusinessRecommendedPluginController(BaseActivity activity, BusinessRecommendedViewPlugin parent) {
	...
	private void setOtherIdolView() {
		//		SpaceVPAdapter<ZMObject> adapter = new SpaceVPAdapter<ZMObject>(mParentActivity, mCacheZMObjectList, preview_size,
		//				item_count) {
		//			@Override
		...
		if (drawableCardAdapter == null) {
			drawableCardAdapter = new DrawableCardAdatper(mParentActivity, R.layout.space_darwable_card_item,
					mCacheZMObjectList);
			mViewPlugin.setAdapter(drawableCardAdapter);
			mViewPlugin.getGallery().setSelection(2);
//			mViewPlugin.setOnItemClickListener(new OnCardItemClickListener() {
//				@Override
...
//								drawableCardAdapter.getItem(position), true);
...
//			});
			...
			drawableCardAdapter.setData(mCacheZMObjectList);
