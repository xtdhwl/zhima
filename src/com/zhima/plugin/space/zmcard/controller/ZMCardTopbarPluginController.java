...
/**
 * @ClassName: ZMCardTopbarPluginController
 * @Description:名印空间标题栏
 * @author luqilong
 * @date 2013-1-5 上午11:36:33
 * 
 */
public class ZMCardTopbarPluginController extends BaseViewPluginController {

	private TopbarBaseViewPlugin mTopbarViewPlugin;
	private ZMCardObject mZMcardObject;

	public ZMCardTopbarPluginController(BaseActivity activity, BaseViewPlugin parent) {
	...
	public void loadData(ZMObject zmObject, boolean refreshed) {
		...
		//判断是否为自己 
		User zmUser = mZMcardObject.getUser();
	...
	public void favoriteTopbaClick(View v) {
		if (mZMcardObject != null) {
			...
			entry.setTitle(mZMcardObject.getName());
			entry.setObjectType(mZMcardObject.getZMObjectType());
			entry.setObjectId(mZMcardObject.getRemoteId());
			entry.setTelephone(mZMcardObject.getTelephone());
			entry.setImageUrl(mZMcardObject.getImageUrl());
			ContactService.getInstance(mParentActivity).addContact(entry, true, ZMCardTopbarPluginController.this);
		} else {
				...
				menu.setOnMenuItemClickListener(new OnMenuItemClickListener() {
					...
					public void onMenuItemClick(ZhimaMenuItem item, int position) {
						...
						String shareContent = String.format(mParentActivity.getString(R.string.share_content),
								mZMcardObject.getName(), mZMcardObject.getZMID(), mZMcardObject.getSpaceHomepage());
	...
	private View.OnClickListener noticeTopbarClick = new View.OnClickListener() {
		...
		public void onClick(final View v) {
			if (mZMcardObject != null) {
				...
				it.putExtra(BaseActivity.ACTIVITY_EXTRA, mZMcardObject.getZMCode());
				startActivity(it);
			} else {
				mHandler.sendEmptyMessage(IntentMassages.SHOW_ERROR_TOAST);
			}
		...
		if (protocol.isHttpSuccess()) {
			if (protocol.isHandleSuccess()) {
				if (protocol.getProtocolType() == ProtocolType.ADD_CONTACT_PROTOCOL) {
					HaloToast.show(mParentActivity, R.string.save_success);
				}
			} else {
