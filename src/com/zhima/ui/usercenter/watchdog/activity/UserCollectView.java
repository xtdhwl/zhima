 ...
 * 个人管家 收藏 view
 * 
 * @ClassName: UserCollectView
 * @Description: TODO
 * @author yusonglin
 * @date 2013-1-18 下午12:13:34
 */
public class UserCollectView {
	...
	private int mCurrentType = PersonRecordType.IDOL;
	...
	public UserCollectView(Context context, View view) {
		// super(context);
	 ...
	 * @return void
	 */
	public void searchCollect(String keyString) {
		...
		mUserService.searchUserRecord(mKeyString, mCurrentType, 0, true, callBack);
	}
	...
	private void init() {
		...
		mLoadingbr = (LinearLayout) mContentView.findViewById(R.id.collect_view_loading);
		favoriteListView = (CustomListView<UserRecordEntry>) mContentView.findViewById(R.id.list_collect);
	 ...
	 * @return void
	 */
	public void requestServiceData(int currentType) {
		mCurrentMode = ALL_MODE;
		mCurrentType = currentType;
		refreshListDate = mUserService.getCacheUserRecordList(currentType);
		favoriteListView.addLoading();
		setListBatchLoad(currentType);
		...
		if (refreshListDate.isEmpty()) {
			...
			mUserService.getUserRecordList(currentType, getBeginTime(), getEndTime(), 0, true, callBack);
		} else {
			addCacheData();
		}
	}
		...
		favoriteListView.setBatchLoad(new ZhimaAdapter<UserRecordEntry>(mContext, R.layout.space_joke_list_item,
			...
			public void getData() {
				...
				if (mCurrentMode == ALL_MODE) {
					...
					mUserService.searchUserRecord(mKeyString, recordType, 0, false, callBack);
				}
			...
			if (protocol.isHttpSuccess()) {
				if (protocol.getProtocolType() == ProtocolType.GET_USER_RECORD_LIST_PROTOCOL) {
					...
					if (protocol.isHandleSuccess()) {
						...
						favoriteListView.mAlreadyLoadData.clear();
						favoriteListView.addLoading();
						mUserService.getUserRecordList(mCurrentType, getBeginTime(), getEndTime(), 0, true, callBack);
					} else {
						HaloToast.show(mContext, protocol.getProtocolErrorMessage());
					}
	...
	private void showContentEmpty(boolean bl, boolean noNet) {
		TextView txt = (TextView) mContentView.findViewById(R.id.collect_txt_empty);
		txt.setVisibility(bl ? View.VISIBLE : View.GONE);
		if (noNet) {
			txt.setText(R.string.myzhima_request_failed);
		} else if (mCurrentType == PersonRecordType.IDOL) {
			txt.setText(R.string.no_favorite_ido);
		} else if (mCurrentType == PersonRecordType.WEDDING) {
			txt.setText(R.string.no_favorite_couple);
		} else if (mCurrentType == PersonRecordType.FAVORITE) {
			txt.setText(R.string.no_favorite_data);
		}
