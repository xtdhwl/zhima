...
public class HttpImageLoader {
	...
	private static HttpImageLoader mHttpImageLoader = null;
	...
	public static HttpImageLoader getInstance(Context c) {
		if (mHttpImageLoader == null) {
			mHttpImageLoader = new HttpImageLoader();
		}
	...
	//	private Bitmap reloadImage(String uri, ImageView view, int maxWidth, int maxHeight, long belongId, int defaultId) {
	//		Bitmap bmp = loadImage(uri, view, false, maxWidth, maxHeight, belongId, defaultId, true);
	...
	//		return loadImage(uri, view, belongId, defaultId, "");
	//	}	
	...
	public Bitmap loadImage(String uri, ImageView view, boolean isBackground, int maxWidth, int maxHeight,
			long belongId, int defaultId, boolean reload, HttpImageListener listener, String scaleType) {
	...
	private synchronized Bitmap doLoadImage(String uri, ImageView view, boolean isBackground, int maxWidth,
			int maxHeight, long belongId, int defaultId, boolean reload, HttpImageListener listener, String scaleType) {
		...
		if (TextUtils.isEmpty(scaleType)) {
			realUrl = ResourceServerConfig.getInstance().getRealImageUrl(uri, maxWidth);
		} else {
			realUrl = ResourceServerConfig.getInstance().getRealImageUrl(uri, scaleType);
		}
		...
		if (realUrl != null && view != null) {
			...
			Bitmap bmp = mImageCache.getBitmapFromSoft(realUrl);
			...
			try {
				...
				if (imageFile != null) {
					if (!mOnlyDownload) {
						...
						try {
							mBitmap = GraphicUtils.getImageThumbnailByPixel(mContext, imageFile, mImageWidth,
									mImageHeight);
...
//		}
	...
	private void addViewHolder(ImageHolder holder) {
		...
		if (holders == null) {
			holders = new ArrayList<HttpImageLoader.ImageHolder>();
		}
		holders.add(holder);
