...
public class PhotoAdapter extends BaseAdapter {
	...
	private String imageScaleType = ImageScaleType.ORIGINAL;

	public PhotoAdapter(Context context, List<ZMObjectImage> array) {
		mContext = context;
		mArray = array;
	...
	public PhotoAdapter(Context context, List<ZMObjectImage> array,String imageScaleType) {
		mContext = context;
		mArray = array;
		mImageCache = new SoftReferenceCache();
	...
	@Override
	public ZMObjectImage getItem(int position) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ZMObjectImage data = mArray.get(position);
		...
		String realUrl = ResourceServerConfig.getInstance().getRealImageUrl(url,imageScaleType);
		...
		if (bitmap != null) {
			...
			HttpImageLoader.getInstance(mContext).downloadImage(url,imageScaleType,
					new MyImageDownloadListener(view));
		}
		return view;
	}
