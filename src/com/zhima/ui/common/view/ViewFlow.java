...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private final static int TOUCH_STATE_SCROLLING = 10;
	...
	/**是否左右移动*/
	private boolean mIsMoveLeftOrRight;
	/**是否上下移动*/
	private boolean mIsMoveUpOrDown;
	//------------------------
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			mLastMotionX = x;
			mLastMotionY = y;
			mIsMoveLeftOrRight = false;
			mIsMoveUpOrDown = false;
			//修改TOUCH_STATE_SCROLLING为10 判断左右or上下移动的差距
		...
		case MotionEvent.ACTION_MOVE:
			...
			//优先判断上下 然后判断左右
			//------------------------------------------------
			//如果y轴符合上下移动 并且 没有左右移动  --》确定为上下移动
			if (yMoved && !mIsMoveLeftOrRight) {
				mIsMoveUpOrDown = true;
			}
			//如果x轴符合左右移动 并且 没有上下移动  --》确定为左右移动
			if (xMoved && !mIsMoveUpOrDown) {
				// Scroll if the user moved far enough along the X axis
				mTouchState = TOUCH_STATE_SCROLLING;
				mIsMoveLeftOrRight = true;
				if (mViewInitializeListener != null)
					initializeView(deltaX);
			}
			
			//如果可以上下移动 或 左右移动
			if (mIsMoveLeftOrRight || mIsMoveUpOrDown) {
				// Scroll to follow the motion event

				mLastMotionX = x;
				mLastMotionY = y;

				if (mIsMoveLeftOrRight) {
