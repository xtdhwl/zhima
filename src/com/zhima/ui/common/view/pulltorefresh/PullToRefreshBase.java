...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	static final boolean DEBUG = SystemConfig.DEBUG;
...
//	}
	 ...
	 * 
	 * @param disableScrollingWhileRefreshing - true if you want to disable
	 ...
	 * 
	 * @param releaseLabel - String to display
	 * @param mode - Either <code>MODE_PULL_DOWN_TO_REFRESH</code>,
	 *            <code>MODE_PULL_UP_TO_REFRESH</code> or <code>MODE_BOTH</code>
	...
	public void setLastUpdatedLabel(String label) {
		mFooterLayout.setSubLabel(label);
	}
	...
	/**
	 * @Title: setLastPage
	 * @Description:是否为最后一页。如果为最后一页，页脚显示没有更多,并且再次上拉不再加载回调.
	 * @param bl
	 */
	...
	public void onRefreshComplete(boolean bl) {
		onRefreshComplete();
	}
			...
			if (mIsBeingDragged) {
				...
				if (mState == RELEASE_TO_REFRESH) {
					...
					if (null != mOnRefreshListener) {
						//修改：当前为最后一页并且是上拉，则上拉不回调
						if (mIsLastPage && mCurrentMode == MODE_PULL_UP_TO_REFRESH) {
							onRefreshComplete();
							return true;
						}
	...
	//修改：getLayout方法。因为header与footer不是完全相同
...
//	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mMode == MODE_PULL_DOWN_TO_REFRESH || mMode == MODE_BOTH) {
			mHeaderLayout = new HeaderLoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH);
		...
		if (mMode == MODE_PULL_UP_TO_REFRESH || mMode == MODE_BOTH) {
			mFooterLayout = new FooterLoadingLayout(context, MODE_PULL_UP_TO_REFRESH);
